...
class TeFu_Adapter(nn.Module):
      # Text-guided Fusion Adapter
      def __init__(self, cfg):
            super(TeFu_Adapter, self).__init__()
      ...
      # clip_adapter = nn.Linear(clip_cache_keys.shape[0], clip_cache_keys.shape[1], bias=False).to(clip_model.dtype).cuda()
            ...
            self.adapter_layer = nn.Sequential(
                        nn.Linear(
                        in_features=3072, # if clip_backbone = RN50, in_features=3072; if clip_backbone = RN101, ViT-B/32, ViT-B/16, in_features=2560; 
                        out_features=1024,
                        bias=False),
      ...
      def forward(self, fea1, fea2):
            fea1 = fea1.float()
            fea = torch.cat((fea1, fea2), dim=1) # bathcsize, dim
